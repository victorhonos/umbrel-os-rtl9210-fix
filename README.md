...
The problem is to run local server with Umbrel **☂️ Umbrel OS** (Raspberry Pi 4) using **External SSD enclosure** with chipsets ***RTL9210 and RTL9210B*** (Orico, Ugreen, and more). Many of enclosures contains old version of firmware, is unstable and bring problems in Umbrel OS system. So, try to use a enclosure with these chipsets, often cause message ***Refused connection*** in `https://umbrel.local` after some time. This occurs because the default firmware force the ***Sleep Mode/Enclosure suspension*** from time to time. When Umbrel OS try access and write disk finds device *sleeping*, doing to lock the *Docker* and others services of Umbrel OS, repeating this cicle with every forced restart system.
...
The solution is to *flash* a new stable firmware version (**Recommended v1.32.49**) and inject a custom config file (.cfg), that permanently (until you return to default version) disables any power-saving features from the enclosure. This brings full availability of your SSD NVMe/SATA to your Umbrel OS (especially if you plan to run a Bitcoin Node).
...
> **Info:** The firmware update tool is **ONLY AVALIABLE FOR WINDOWS** and *does not work well on Linux with Wine or in a virtual machine*.
